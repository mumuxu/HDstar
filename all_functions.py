...
def n2g(nodex,nodey,xstep,ystep):
    gpsx= (nodex-1.0)*xstep + goalgps[0]
    gpsy= (nodey-1.0)*ystep + goalgps[1]
    gps=[gpsx,gpsy]
    return gps

def g2n(gpsx,gpsy,xstep,ystep):
    nodex= (gpsx-goalgps[0])/xstep+1
    nodey= (gpsy-goalgps[1])/ystep+1
    node=[int(nodex),int(nodey)]
    return node
##################################################################################################################################
...
def lineOfSight(*args):
    ...
    sx, sy, sz = (dx>0)*1-(dx<0)*1,     (dy>0)*1-(dy<0)*1,    (dz>0)*1-(dz<0)*1     # (a > b) - (a < b replaces cmp(a,b)) (This program is written in Python 2.x)
    if ax >= max(ay,az):
        yD = ay - ax/2
        zD = az - ax/2
        while x1 != x2:
...
def setupLevels():
    ...
    # Get dimensions for each level       
        if level == 0:
            ...
            if lsizeX > sizeX/gl.minclustersize:  lsizeX = sizeX/gl.minclustersize            ###This allows lsizeX,Y,Z to possibly be zero because how int behaves
...
def searchAndUpdate_old(xNew,yNew,zNew,*args):
    ...
    for obsLoc in searchRange:
        if gl.map_[obsLoc] == - 2 or gl.map_[obsLoc] == -1:
            ...
            gl.map_[obsLoc] = -1                          #add the obstacle to known obstacles?
            gl.costMatrix[obsLoc] = float('inf')           #make the location of that obstacle have a an infinite cost 
                                                           # (is this node by node or is location of obstacle a set of all the nodes in the obstacle?
